...
export default function TasksScreen() {
  ...
  const tareasFiltradas = tasks.filter((t) => (filtroEstado === 'todas' ? true : t.estado === filtroEstado));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <Screen bottomExtra={72} refreshing={isLoading} onRefresh={refetch}>
        <AppText variant="display" style={{ marginBottom: spacing.lg }}>Tareas</AppText>

        {/* Métricas */}
        <View style={{ flexDirection: 'row', gap: spacing.md, marginBottom: spacing.lg }}>
          <StatCard label="Pendientes" value={String(pendientes)} icon="flash-outline" accent={colors.tasks} accentSoft={colors.tasksSoft} footnote="Por completar" />
          <StatCard label="Completadas" value={String(completadas)} icon="checkmark-done-outline" accent={colors.success} accentSoft={colors.successSoft} footnote="Finalizadas" />
        </View>

        {/* Filtros */}
        <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.lg }}>
          {filtros.map((f) => (
            <Chip key={f.key} label={f.label} active={filtroEstado === f.key} onPress={() => setFiltroEstado(f.key)} activeColor={colors.tasks} flex />
          ))}
        </View>
        ...
        {tareasFiltradas.length === 0 ? (
          <Card>
            <EmptyState
              ...
              subtitle={filtroEstado === 'todas' ? 'Pulsa el botón + para crear la primera tarea del hogar.' : undefined}
            />
          </Card>
        ) : (
          tareasFiltradas.map((item) => {
            ...
            return (
              <Card key={item.id} padding={spacing.lg} style={{ marginBottom: spacing.md, flexDirection: 'row', alignItems: 'center' }}>
                {/* Checkbox */}
                <Pressable
                ...
                <View style={{ flex: 1, marginRight: spacing.sm }}>
                  <AppText variant="bodyStrong" numberOfLines={2} color={completada ? colors.inkFaint : colors.ink} style={completada ? { textDecorationLine: 'line-through' } : undefined}>
                    {item.nombre}
                  </AppText>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 5, flexWrap: 'wrap' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                      <Icon name={getFrecuenciaIcon(item.frecuencia)} size={12} color={colors.inkFaint} />
                      <AppText variant="micro" color={colors.inkMuted} style={{ textTransform: 'capitalize' }}>{item.frecuencia}</AppText>
                    </View>
                    {item.asignado_a ? (
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                        <Icon name="person-outline" size={12} color={colors.inkFaint} />
                        <AppText variant="micro" color={colors.inkMuted}>{item.asignado_a}</AppText>
                      </View>
                    ) : null}
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 2, borderRadius: radius.pill, backgroundColor: prio.soft }}>
                      <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: prio.color }} />
                      <AppText variant="micro" color={prio.color} style={{ fontWeight: '700' }}>{prio.label}</AppText>
                    </View>
                  </View>
                </View>

                {/* Eliminar */}
                <IconButton name="trash-outline" size={17} color={colors.danger} bg={colors.dangerSoft} diameter={34} onPress={() => handleDelete(item)} accessibilityLabel="Eliminar tarea" />
              </Card>
            );
          })
        )}
      ...
      <Fab icon="add" color={colors.tasks} onPress={() => setModalVisible(true)} accessibilityLabel="Añadir tarea" />
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: colors.overlay }}>
          <View style={{ backgroundColor: colors.card, borderTopLeftRadius: radius.xxl, borderTopRightRadius: radius.xxl, padding: spacing.xl, paddingBottom: spacing.xxxl, maxHeight: '88%' }}>
            ...
            <AppText variant="h2" style={{ marginBottom: spacing.lg }}>Nueva tarea</AppText>
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <Field label="Nombre *" placeholder="Ej: Sacar la basura" value={nombre} onChangeText={setNombre} />
              <Field label="Asignado a" placeholder="Ej: Mamá (opcional)" value={asignadoA} onChangeText={setAsignadoA} />

              <AppText variant="label" color={colors.inkMuted} style={{ marginBottom: 6 }}>Frecuencia</AppText>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginBottom: spacing.lg }}>
                {FRECUENCIAS.map((f) => (
                  <Chip key={f} label={f} active={frecuencia === f} onPress={() => setFrecuencia(f)} activeColor={colors.tasks} />
                ))}
              </View>

              <AppText variant="label" color={colors.inkMuted} style={{ marginBottom: 6 }}>Prioridad</AppText>
              <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.xl }}>
                {PRIORIDADES.map((p) => (
                  <Chip key={p} label={p} active={prioridad === p} onPress={() => setPrioridad(p)} activeColor={colors.tasks} flex />
                ))}
              </View>

              <Button label="Crear tarea" icon="add" onPress={handleAdd} style={{ marginBottom: spacing.sm }} />
              <Button label="Cancelar" variant="ghost" onPress={() => { setModalVisible(false); resetForm(); }} />
            </ScrollView>
          </View>
        </View>
      </Modal>
